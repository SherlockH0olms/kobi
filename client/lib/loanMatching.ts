...
const MOCK_LOAN_OFFERS: LoanOffer[] = [
  ...
  },
  {
    ...
    minCreditScore: 70,
    features: ['Best rates', 'Dedicated account manager', 'Early repayment allowed'],
  ...
  },
  {
    ...
    minCreditScore: 75,
    features: ['Lowest rates', 'Large loan amounts', 'Expert business consultation'],
    processingTime: '2-3 business days',
  },
  {
    id: 'loan-006',
  ...
  },
  {
    ...
    repaymentPeriodMonths: 48,
    minCreditScore: 65,
    features: ['Tech company focused', 'Mentorship included', 'Lowest tech rates'],
    processingTime: '3-5 business days',
  },
  {
    id: 'loan-008',
...
export function getEligibleLoans(creditScore: number, limit?: number): LoanOffer[] {
  // Filter loans based on credit score requirement
  const eligible = MOCK_LOAN_OFFERS.filter(
    loan => creditScore >= loan.minCreditScore
  );

  // Sort by interest rate (ascending) to show best deals first
...
export function sortLoansByAmount(loans: LoanOffer[]): LoanOffer[] {
  return [...loans].sort((a, b) => a.loanAmountMax - b.loanAmountMax);
}

export function sortLoansByRepaymentPeriod(loans: LoanOffer[]): LoanOffer[] {
  return [...loans].sort((a, b) => a.repaymentPeriodMonths - b.repaymentPeriodMonths);
}

export function filterLoansByAmount(
  loans: LoanOffer[],
  minAmount: number,
  maxAmount: number
): LoanOffer[] {
  return loans.filter(
    loan => loan.loanAmountMax >= minAmount && loan.loanAmountMin <= maxAmount
  );
}

export function filterLoansByInterestRate(
  loans: LoanOffer[],
  maxRate: number
): LoanOffer[] {
  return loans.filter(loan => loan.annualInterestRate <= maxRate);
}

// Get recommendation score for a loan based on company data
export function getLoanRecommendationScore(
  ...
  const requestedAmount = companyData.annualRevenue * 0.3; // Request ~30% of annual revenue
  if (
  ...
  // Score based on credit score margin (better if higher margin)
  const scoreMargin = creditScore - loan.minCreditScore;
  score += Math.min(20, scoreMargin / 5);

  // Shorter repayment periods for better cash flow
