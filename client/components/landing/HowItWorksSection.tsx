...
function Step({ number, title, description, isReverse = false, children }: StepProps) {
  const content = (
    <>
      {/* Number Circle */}
      <div className="mb-8">
        <div className="w-28 h-28 rounded-full bg-success/10 flex items-center justify-center mb-6">
          <span className="text-5xl font-heading font-bold text-success">{number}</span>
        </div>

        {/* Text Content */}
        <h3 className="text-3xl font-heading font-bold text-foreground mb-4">{title}</h3>
      ...
      <div className="bg-white rounded-2xl border border-border p-8 shadow-lg max-w-sm w-full">
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div className="text-sm font-semibold text-foreground">Ahead of the #AI3</div>
          ...
          <div className="space-y-2">
            {[1, 2, 3].map(i => (
              <div key={i} className="h-2 bg-muted rounded w-3/4" style={{ opacity: 1 - i * 0.2 }} />
            ))}
          </div>
        </div>
      </div>
  ...
  return (
    <div className={`grid grid-cols-1 lg:grid-cols-2 gap-12 items-center ${isReverse ? 'lg:auto-cols-max' : ''}`}>
      {isReverse ? (
        <>
          {visual}
          <div>{content}</div>
    ...
    <div className="bg-gradient-to-b from-muted/30 to-background py-20 sm:py-28">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-20">
          <h2 className="text-4xl sm:text-5xl font-heading font-bold text-foreground mb-4">Necə işləyir?</h2>
